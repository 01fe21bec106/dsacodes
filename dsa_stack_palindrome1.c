   ...
   { char data[SIZE];
    ...
    void push(struct stack *sptr, char num);
     void check(struct stack *sptr,char no[SIZE]);
      ...
      void push(struct stack *sptr, char num)
      { if(sptr->top==SIZE-1) { printf("Stack Overflow\n");
       } else { sptr->top++; sptr->data[sptr->top]=num; } }
        char pop(struct stack *sptr)
         { int num;
          ...
          sptr->top--;
           return num; }
           void check(struct stack *sptr,char no[SIZE])
           { int ch,stat=1; int i=0; char temp; while(no[i]!='\0')
           { ch=no[i];
           push(sptr,ch);
           i++; }
           i=0;
             ...
             if(temp!=pop(sptr))
              { stat=0; break;
               ...
               { printf("%s is a palindrome string",no);
                ...
                { struct stack * sptr; struct stack s;
                 char pa[SIZE];
                 sptr=&s;
                  sptr->top=-1;
                   scanf("%s",pa);
                 check(sptr,pa); return 0;
                  }
